...
void FUpdateLightBufferPass::BakeShadowMap(FRenderingContext& Context)
{
    ...
    // If CSM is enabled, use refactored path and early-exit
    const bool bUseCSM = (Context.ShowFlags & EEngineShowFlags::SF_CSM) != 0;
    if (bUseCSM)
    {
        RenderDirectionalCSM(Context);
        // End shadow-bake and restore viewport/targets
        EndShadowBake(DeviceContext, SavedViewport);
...
void FUpdateLightBufferPass::BeginShadowBake(ID3D11DeviceContext* DeviceContext,
...
void FUpdateLightBufferPass::EndShadowBake(ID3D11DeviceContext* DeviceContext,
...
void FUpdateLightBufferPass::RenderDirectionalShadowSimple(FRenderingContext& Context)
{
    ...
    FVector LightFwd = DirectionalLightComponent->GetForwardVector();
    ...
    if (!DirectionalLightComponent->GetCastShadows())
    {
        ...
        LVPShadowMap.LightDirWS      = (-DirectionalLightComponent->GetForwardVector()).GetNormalized();
    ...
    else
    {
        ...
        FitLightToNDCBox(DirectionalLightComponent->GetForwardVector(), ndcMin, ndcMax, V_L_prime, P_L_prime, orthoLTRB, nearPlane, farPlane);
        ...
        FVector LdirWS = -DirectionalLightComponent->GetForwardVector(); // 라이트 광선 방향이 +Fwd라면, 표면→광원은 -Fwd
        ...
        PSM.ShadowParams = FVector4(DirectionalLightComponent->GetShadowBias(), DirectionalLightComponent->GetShadowSlopeBias(), 0, 0);
