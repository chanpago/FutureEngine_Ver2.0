...
void FStaticMeshPass::Execute(FRenderingContext& Context)
{
	...
	*/
	...
	// +-+ RTVS SETUP +-+
	// Bind or clear point shadow 3-Tier resources based on shadow flag
	{
		FUpdateLightBufferPass* LightBufferPass = dynamic_cast<FUpdateLightBufferPass*>(Renderer.GetRenderPasses()[0]);
		...
		{
			// Keep tier mapping bound (now invalidated to disable shadows), clear heavy textures
			Pipeline->SetShaderResourceView(14, EShaderType::PS, LightBufferPass ? LightBufferPass->GetPointShadowCubeIndexSRV() : nullptr);
			for (int slot = 15; slot <= 23; ++slot)
			{
				Pipeline->SetShaderResourceView(slot, EShaderType::PS, nullptr);
			}
		}
	}
